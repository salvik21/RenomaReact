...
function HamburgerSubenu({ onClickBackButton, HamburgerSubmenuClass, refSubmenu }) {
    const LinkElement = [
        ...
        <div ref={refSubmenu} className={`hamburger-submenu-wrapper ${HamburgerSubmenuClass}`} >
            <div className="hamburger-submenu-back-button-wrapper">
                <div className="hamburger-submenu-image-wrapper">
                    <Image imageClass={"back-black-image"} imageSrc={BackBlackButton} imageAlt={"Back button"} imageOnClick={onClickBackButton} />
                </div>
                <Label labelClass={"submenu-back-button-title"} text={"BACK"} />
            </div>
            {LinkElement.map((item) =>
                <div key={item.id + 100} className="hamburger-menu-buttons-wrapper">
